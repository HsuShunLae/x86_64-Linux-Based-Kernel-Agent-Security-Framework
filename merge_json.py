    ...
    for policy in (policy_a, policy_b):
        for frm, to, cnt in policy.get("transition_counts", []):
            if frm in allowed_set and to in allowed_set:
                merged_transition_counts[(frm, to)] = (
                    merged_transition_counts.get((frm, to), 0) + cnt
                )
    ...
    for policy in (policy_a, policy_b):
        for nr, cnt in policy.get("syscall_counts", []):
            if nr in allowed_set:
                merged_syscall_counts[nr] = merged_syscall_counts.get(nr, 0) + cnt
...
def main():
    parser = argparse.ArgumentParser(
        description="Merge syscall policies: intersection allowlist, union+pruned FSM"
    )
    ...
    print(f"     allowed_syscalls : {len(merged['allowed_syscalls'])}")
    print(f"     entry_syscalls   : {len(merged['entry_syscalls'])}")
    print(f"     transitions      : {len(merged['transition_counts'])}")
